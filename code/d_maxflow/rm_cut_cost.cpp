...
void read_cut(const char * name, int *& S, double & F0, int & n){
...
double cut_cost(const char * problem, int * S, int n){
...
int main(int argc, char *argv[]){
	...
	int n;
	read_cut(cut,S,F0,n);
	double F = cut_cost(problem,S,n);
	if(F==F0){
		printf("Cut cost confirmed, %f\n",F);
	}else{
		printf("Error: claimed flow, %f, differs from the cut cost, %f\n",F0, F);
	};
	if(S)delete S;
};
